...
const month = 8;
let counter = 0;
export function getGreeting(daysData, year, month, day) {
    ...
    const ocaisions = monthsInDaysData.map(event => { // it will be a list of objects with dayName and occurance and occurenceIndex
        ...
        return {dayName: event.dayName, dayIndex: dayIndex, occurence: event.occurence, occuranceIndex: occurenceIndex}
    });
...
console.log(getGreeting(daysData, year, 8, 1))
const weekDays = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
